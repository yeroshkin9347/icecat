...
const MappingEntry = React.memo(({ source, target, onChange }) => {
    return (
        <div className="row">
            <div className="col-6 pad_shrink_left">
                <input
                    placeholder="Source"
                    ...
                    onChange={(e) =>
                        onChange({
                            source: e.currentTarget.value ? e.currentTarget.value : null,
                        })
                    }
                />
            </div>
            <div className="col-6 pad_shrink_right">
                <input
                    placeholder="Target"
                    className="form-control"
                    value={target || ""}
                    onChange={(e) =>
                        onChange({
                            target: e.currentTarget.value ? e.currentTarget.value : null,
                        })
                    }
                />
            </div>
...
const DictionaryParam = ({
    ...
    for (var obj in value) mappings.push({ source: obj, target: value[obj] });
    
    return (
        <React.Fragment>
            <div className="form-group">
                {map(mappings, (mappingEntry, mappingEntryIdx) => (
                    <div
